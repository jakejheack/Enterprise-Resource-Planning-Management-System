...
(function ($) {
    ...
    function saveClient() {
        ...
        abp.message.confirm(
            'Client ' + $c  +' will be updated.',
            'Are you sure?',
            function (isConfirmed) {
                if (isConfirmed) {
                    abp.ui.setBusy(_$form);
                    _clientService.updateClient(client).done().always(function () {
                        abp.ui.clearBusy(_$form);
                        abp.notify.success('Client ' + $c + ' updated successfully : ' + $name, 'Success');
                        //$.ajax({
                        //    type: 'GET',
    ...
    function loadPage() {
        ...
        switch (statusId) {
            case '1':
                ...
                if ($('#SaveClientButton').length) {
                    $('#SaveClientButton').removeAttr('hidden');
                }
                if ($('#ApproveButton').length) {
                    $('#ApproveButton').removeAttr('hidden');
                }
                if ($('#DisapproveButton').length) {
                    $('#DisapproveButton').removeAttr('hidden');
                }
                break;
            case '2':
