...
(function () {
    $(function () {
        ...
        var _$table = $('#CVTable');
        var _$table2 = $('#CVTable2');
        var _service = abp.services.app.cVService
        ...
        var dataTable = _$table.DataTable({
            ...
            searching: false,
            listAction: {
                ajaxFunction: _service.getAllCV,
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    "render": function (data) {
                        return '<a id="view-cv" title="view" href="#" class="view-cv" data-cv-id="' + data.id + '" data-cv-code="' + data.code + '">' + data.code + '</i></a>';
                    }
                }
                ,
                {
...

                {
                    ...
                    "render": function (data) {
                       // return '<a id="edit-rfp" title="edit" href="#" class="edit-rfp" data-rfp-id="' + data.id + '" data-rfp-code="' + data.code + '"  data-rfp-status="' + data.status + '" data-toggle="modal" data-target="#ClientEditModal"  ><i class="fa fa-pencil-square-o"></i></a>|<a id="delete-rfp" title="delete" href="#" class="delete-rfp" data-rfp-id="' + data.id + '"><i class="fa fa-trash"></i></a>';
                        //return '<i class="fa fa-trash"></i></a>';
                        var edit = '<a id="edit-cv" title="edit" href="#" class="edit-cv" data-cv-id="' + data.id + '" data-cv-code="' + data.code + '"><i class="fa fa-pencil-square-o"></i></a>';
                        //var del = '<a id="delete-cv" title="delete" href="#" class="delete-cv" data-cv-id="' + data.id + '"><i class="fa fa-trash"></i></a>';
                        var ret = '';
                        if (abp.auth.isGranted("Pages.Cv.Edit")) {
                            ret += edit;
                        }
                        //if (abp.auth.isGranted("Pages.Cv.Delete")) {
        ...
        var dataTable2 = _$table2.DataTable({
         ...
         //View record
        _$table.on('click', 'a.view-cv', function (e) {
            e.preventDefault();
            var clientId = $(this).attr("data-cv-id");
            window.location.href = abp.appPath + 'CV/Details?id=' + clientId;
        });


        _$table.on('click', 'a.edit-cv', function (e) {
            e.preventDefault();
            var rfqid = $(this).attr("data-cv-id");
            var Code = $(this).attr("data-cv-code");
            var rfqstatus = $(this).attr("data-rfq-status");
            abp.message.confirm(
                abp.utils.formatString(abp.localization.localize('Edit Check Voucher. ' + Code, 'ezinvmvc'), name),
                function (isConfirmed) {
                    if (isConfirmed) {
                        window.location.href = abp.appPath + 'CV/Edit?id=' + rfqid;
                    }
                }
            );
        });
        ...
        _$table.on('click', 'a.delete-cv', function (e) {
            var id = $(this).attr("data-cv-id");
        ...
        _$table.on('click', 'a.edit-rfp', function (e) {
            ...
            abp.message.confirm(
                ...
                function (isConfirmed) {
                    if (isConfirmed) {
                        window.location.href = abp.appPath + 'CV/Edit?id=' + Id;
                    }
                }
            );
        });
        ...
        $('#ExportToExcelButton').click(function (e) {
            e.preventDefault();
            tableToExcel('CVTable2', 'Check Voucher', 'CheckVoucher.xls');
        });
