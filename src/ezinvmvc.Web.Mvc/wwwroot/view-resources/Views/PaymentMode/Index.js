...
(function () {
    $(function () {
        ...
        var dataTable = _$table.DataTable({
            ...
            searching: false,
            listAction: {
                ajaxFunction: _service.getPaymentModes
            },
            columnDefs: [
                {
                ...
                },
                {
                    ...
                    "render": function (data) {
                        ...
                        var ret = '';
                        if (abp.auth.isGranted("Pages.PaymentMode.Edit")) {
                            ret += edit;
                        }
        ...
        function getPaymentMode(id) {
            _service.getPaymentMode({ id: id }).done(function (result) {
                $('#NameE').val(result.name);
                //$('#Ename').val(result.name);
                $('#DefaultAccountIdE').val(result.defaultAccountId);
                $('input[type=checkbox][name=IsTaxE]').prop('checked', result.isTax);
        ...
        _$formCreate.find('button[type="submit"]').click(function (e) {
            ...
            abp.message.confirm(
                'New Payment Mode will be added.',
                'Are you sure?',
                function (isConfirmed) {
                    if (isConfirmed) {
                        abp.ui.setBusy(_$modalCreate);
                        _service.createPaymentMode(create).done(function () {
                            abp.notify.success('Payment Mode created', 'Success');
        ...
        function update() {
            ...
            var viewData = {
                ...
                "isTax": isTax,
                "name": formdata.NameE,
                "defaultAccountId": formdata.DefaultAccountIdE
            };
            disabled.attr('disabled', 'disabled');

            abp.message.confirm(
                'Payment Mode will be updated.',
                'Are you sure?',
                function (isConfirmed) {
                    if (isConfirmed) {
                        abp.ui.setBusy(_$modalEdit);
                        _service.updatePaymentMode(viewData).done(function () {
                            abp.notify.success('Payment Mode has been successfully updated', 'Success');
