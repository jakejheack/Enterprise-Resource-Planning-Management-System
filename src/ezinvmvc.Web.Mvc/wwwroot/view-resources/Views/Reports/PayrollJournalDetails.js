...
(function () {
    $(function () {
        ...
        $('#AttendanceTable').on('click', 'a.view-item', function (e) {
            ...
            $('#attid').val(attid);
            $('#period').val(period);
            $('#coveredDate').html("For&nbsp;the&nbsp;Period&nbsp;of&nbsp;:&nbsp;[&nbsp;" + period + "&nbsp;]&nbsp;/&nbsp;[&nbsp;" + attid + "&nbsp;]");
        ...
        function GetJournalPayroll(attids,card, sectors)
        {
            ...
            _payrollIServices.getPayrollJournalDetailList({ filter: attids + "|" + sectors + "|||" + card }).done(function (result) {
                for (var i = 0; i < result.items.length; i++) {
                    ...
                    PayrolldataTable.row.add([itemno, $attId, $description1.toUpperCase(), $description3.toUpperCase(), $empId, $employeeCode, $description2.toUpperCase(), $firstName.toUpperCase(), $lastName.toUpperCase(), $netdisplay, $netIncome]).draw();
                }
            })
        ...
        function printPreviewActual2() {
            ...
            win.document.write('<h4>Payroll Journal Details</h4>');
