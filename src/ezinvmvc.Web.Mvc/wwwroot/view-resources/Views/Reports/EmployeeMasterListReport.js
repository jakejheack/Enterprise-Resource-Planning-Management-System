...
(function () {
    $(function () {
        ...
        function getempmasterlist() {
            ...
            _employeesservice.getAllEmployeeMasterList({ filter: $DateFrom + '|' + $DateTo + '|' + $EmployeeCode + '|' + $Employeename + '|' + $sectors + '|' + $Position + '|' + $StatusId + '|' + $Department }).done(function (result) {
                for (var i = 0; i < result.items.length; i++) {
                    ...
                    dataTable.row.add([$itemno, $sect, $post, $dept.toUpperCase(), $employeeCode, $name.toUpperCase(), $gender.toUpperCase(), $empType.toUpperCase(), $civilStatus, $birthDate, $sss, $philHealthNo, $pagIbigNo, $tin, $bankNo, $hireDate, $Resigned, $managerId +" Yrs", $Stats]).draw();
                }
            });
        }
        //Export
        $('#ExportToExcelButton').click(function (e) {
            e.preventDefault();
            var dt = new Date($.now());
            tableToExcel('EmpTable', 'EmpTable', 'EmployeeMasterListReport.xls');
        });
        ...
        function printPreviewActual() {
            ...
            printContents += '<div style="text-align: left; font-weight: 700; font-size: x-small; width:100%;font-family: sans-serif;">EMPLOYEE MASTER LIST</div>';
